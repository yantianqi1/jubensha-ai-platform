export function renderPlayControls(): string {
  return `<section class="controls panel panel-compact" aria-labelledby="controls-title">
  <h2 id="controls-title">操作入口</h2>
  <div class="control-row">
    <label>API 基础地址<input data-base-url type="url" value="http://127.0.0.1:3001" /></label>
    <button class="button-secondary button-compact" data-action="start-demo">启动雾港 demo</button>
  </div>
  <div class="control-row">
    ...
    <button class="button-secondary button-compact" data-action="create-runtime-room">创建运行房间</button>
  </div>
  <div class="control-row">
    ...
    <button class="button-secondary button-compact" data-action="join-seat">入座</button>
  </div>
  <div class="control-row">
    <button class="button-ghost button-compact" data-action="read-public-snapshot">读取公开快照</button>
    <button class="button-ghost button-compact" data-action="read-seat-snapshot">读取座位快照</button>
  </div>
  <div class="control-row">
    <label>NPC 角色${renderNpcSelect()}</label>
    <label>盘问内容<input data-message type="text" value="窗台怎么回事？" /></label>
    <button class="button-secondary button-compact" data-action="ask-npc">盘问 NPC</button>
  </div>
  <div class="control-row">
    ...
    <button class="button-secondary button-compact" data-action="apply-room-action">提交运行时动作</button>
  </div>
